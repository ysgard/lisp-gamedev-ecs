...
case $1 in
    linux)
        do_build
        DEPLOY_GTK_VERSION=3 linuxdeploy --appimage-extract-and-run --executable=bin/ecs-tutorial \
                    --custom-apprun=package/AppRun \
                    --icon-file=package/icon.png \
                    --desktop-file=package/ecs-tutorial.desktop \
                    --plugin gtk \
                    --appdir=appimage $(find bin -name "lib*" -printf "-l%p ")
        cp bin/ecs-tutorial appimage/usr/bin
        cp -R Resources appimage/usr
        appimagetool --appimage-extract-and-run --comp zstd -g appimage "ecs-tutorial-${VERSION}.AppImage"
        ;;
    ...
    macos)
        do_build
        bundle="ECS Tutorial for Lisp Gamedev.app"
        ...
        xxd -ps -c1 bin/ecs-tutorial | tr -d '\n' | sed "s/$replace_fr/$replace_to/" | fold -w 2 | xxd -r -p > "$contents/MacOS/ecs-tutorial"
        chmod +x "$contents/MacOS/ecs-tutorial"

        hdiutil create -quiet -srcfolder "$bundle" out.dmg
        # NOTE: ULMO = lzma compression = Catalina+ only
        hdiutil convert -quiet out.dmg -format ULMO -o "ecs-tutorial-${VERSION}.dmg"
        rm out.dmg
        ;;
